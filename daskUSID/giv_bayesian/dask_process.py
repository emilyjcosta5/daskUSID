...
from dask.diagnostics import CacheProfiler, ResourceProfiler, visualize
...
class DaskProcess(BaseProcess):
    ...
    def __init__(self, h5_main, *args, verbose=False, **kwargs):
        ...
        # Initialize client:
        #self.client = Client(processes=self.threading)
        #if self.verbose:
        #    print('Core info {}'.format(self.client.ncores()))
    ...
    def compute(self, override=False, *args, **kwargs):
        ...
        L = client.map(self._unit_computation, self.data, *args, **kwargs)
        if self.verbose:
            progress(L)

        with CacheProfiler as cprof, ResourceProfiler as rprof:
            self.data = client.gather(L)
            progress(self.data)
